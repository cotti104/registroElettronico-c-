...
bool verificadataspecifica(int,int,int);
        ...
        if(materia == "mat"){
            ...
            switch(scelta){
                case '1':{
                    ...
                    while(!dataValida(g,m,a) || g > giorni){
                ...
                case '2':{
                    mat.open(materia + "_" + username + ".txt", ios::app);
                    mat << votomat << "=" << ltm->tm_mday << "/" << 1 + ltm->tm_mon << "/" << 1900 + ltm->tm_year;
                    mat.close();
                    break;
                }
                default: break;
...
bool verificadataspecifica(int giorni, int m, int g){
    if(m == 2 && g > giorni){
        return true;
    }
    if(m == 4 && g > giorni){
